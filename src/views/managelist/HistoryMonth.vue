            ...
            <div class="modal-body text-center">
                <div class="container">
                    <div class="row justify-content-center">
                        <div class="col-lg-8">
                            <h2 class="portfolio-modal-title text-secondary text-uppercase mb-0">ประวัติขายสินค้ารายเดือน</h2>
...
export default {
    ...
    gethistorymonth() {
        ...
        var timechoose ;
        moment.locale('nl');
        var monthnow = moment().format('L');
        var month = monthnow.split("-");
        monthchoose = month[1];

        const id = localStorage.getItem('shopid')
        axios.get('https://ezpayapp.azurewebsites.net/api/gethistorymonth/' + id+","+ monthchoose)
        .then(res => {
        ...
        var month = (this.datetimePicker.getMonth()+1) + '-' + (this.datetimePicker.getDate()) + '-' + this.datetimePicker.getFullYear();
        month = moment(month).format('MM');
        console.log(month);
        const id = localStorage.getItem('shopid')
        axios.get('https://ezpayapp.azurewebsites.net/api/gethistorymonth/' + id +","+month)
        .then(res => {
            this.options.id_bill = res.data;
        })
